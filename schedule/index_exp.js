...
const periodChangeSound = new Audio("period-switch.ogg")
periodChangeSound.volume = 0.4
...
function playSound() {
  periodChangeSound.pause()
  periodChangeSound.currentTime = 0
...
function tick() {
  ...
  if (currentPeriod.id === "period" || currentPeriod.id === "transition" || currentPeriod.id === "day-start") {
    if (compareTimeArrs(timeArr, currentPeriod.end) !== -1 && !switching) {
      if (options.audioNotification) playSound()
      console.log(currentPeriod,timeArr,"period change!")
      refreshPeriod(timeArr, currentPeriod.id === "day-start", currentPeriod.id === "period")
      console.log(currentPeriod,timeArr,"period change conclude!")
      return
    }
  }
...
function createOption(label, options, callback, selected, disableDisable) {
	let container = document.createElement("div")
	...
	options.forEach((option)=>{
		...
		p.addEventListener("click", ()=>{
			if (p.classList.contains("selected") && !disableDisable) {
...
function settingsInit() {
  if (localStorage.options) {
    ...
    if (temp.audioNotification === undefined) temp.audioNotification = false
    if (temp.volume === undefined) temp.volume = "0.4"
  ...
  let audioOption = createCheckbox("Period Change Sound", (shouldNotify)=>{
    options.audioNotification = shouldNotify
    if (shouldNotify) periodChangeSound.play()
    settingsSave()
  }, options.audioNotification)
  let volume = createOption("Volume", ["1", "2", "3"], (volumeVal)=>{
    console.log(volumeVal)
    options.volume = ({"1":"0.2","2":"0.4","3":"0.8"}[volumeVal])
    periodChangeSound.volume = options.volume
    playSound()
    settingsSave()
  }, {"0.2":"1","0.4":"2","0.8":"3"}[options.volume], true)
  ...
  generalSettings.appendChild(audioOption)
  generalSettings.appendChild(volume)
}
...
function initDevTools(consoleRun, targetTime) {
  ...
  const devTargetTime = targetTime || [13,47]
  const targetDay = "Wednesday"
